...
class GameState extends EventEmitter {
    ...
    canUseAbility(unit, target) {
        ...
        if (unit.abilityNetherfy) {
            const targets = this.sacrificeList('Drone', this.villain());
            if (targets.length < 1 || !targets[0].abilityUsed) {
                return false;
            }
        }
    ...
    useAbility(unit, target) {
        ...
        if (unit.abilityNetherfy) {
            const targets = this.sacrificeList('Drone', this.villain());
            assert(targets.length > 0 && targets[0].abilityUsed);
            targets[0].sacrificed = true;
        }
    ...
    // Public
    init(deck, baseSets, randomSets, initCards, initResources, infiniteSupplies = false) {
        if (this.turnNumber !== null) {
            throw new InvalidStateError('Already initialized.');
        }

        this.resources = [parseResources(initResources[0]), parseResources(initResources[1])];

        this.deck = deck.map(x => Object.assign(Object.create(DEFAULT_PROPERTIES), x));
        const renames = this.deck.filter(x => x.UIName).reduce((list, x) => {
        ...
        this.supplies = [{}, {}];
        this.initPlayer(0, initCards[0], baseSets[0], randomSets[0], infiniteSupplies);
        this.initPlayer(1, initCards[1], baseSets[1], randomSets[1], infiniteSupplies);
